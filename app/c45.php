...
{ 
	...
	{ 
		...
		$this->rules[$root] = []; 
		if($base != null) { 
			$this->rules[$base][$kasus] = [ 'kasus' => $kasus, 'forward' => $root ]; 
		} 
		...
		foreach($leafs[$root] as $label => $entropy) { 
		 // echo "<h1 style='color:red'>$label $entropy</h1>"; 
			if($entropy == 0) { 
				$this->rules[$root][$label] = [ "kasus" => $label, "nilai" => $labels[$root][$label] ];
			 	 // echo "<h1 style='color:blue'> $root ". $label . ' '.$labels[$root][$label], "</h1>"; 
			} 
			if($entropy > 0 && $entropy <= 1) { 
				if ($base != null) { 
			 	// echo "Roote ".$root; $this->rules[$root][$label] = [ "kasus" => $label.'-', "forward" => $base+1 ]; 
			 	// echo "<hr>", $root; 
				} else { 
